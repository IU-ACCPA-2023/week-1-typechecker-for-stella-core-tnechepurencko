...
let symbol_table = Hashtbl.create 28
let _ = List.iter (fun (kwd, tok) -> Hashtbl.add symbol_table kwd tok)
                  [("µ", SYMB1);(";", SYMB2);(",", SYMB3);("(", SYMB4);(")", SYMB5);("{", SYMB6);("}", SYMB7);("=", SYMB8);(":", SYMB9);("->", SYMB10);("=>", SYMB11);("<", SYMB12);(">", SYMB13);("[", SYMB14);("]", SYMB15);("<=", SYMB16);(">=", SYMB17);("==", SYMB18);("!=", SYMB19);("+", SYMB20);("*", SYMB21);("List::head", SYMB22);("List::isempty", SYMB23);("List::tail", SYMB24);("Nat::pred", SYMB25);("Nat::iszero", SYMB26);("Nat::rec", SYMB27);(".", SYMB28)]

let resword_table = Hashtbl.create 32
let _ = List.iter (fun (kwd, tok) -> Hashtbl.add resword_table kwd tok)
                  [("language", KW_language);("core", KW_core);("extend", KW_extend);("with", KW_with);("fn", KW_fn);("return", KW_return);("type", KW_type);("inline", KW_inline);("throws", KW_throws);("if", KW_if);("then", KW_then);("else", KW_else);("let", KW_let);("in", KW_in);("record", KW_record);("cons", KW_cons);("false", KW_false);("true", KW_true);("succ", KW_succ);("as", KW_as);("match", KW_match);("or", KW_or);("and", KW_and);("not", KW_not);("fix", KW_fix);("fold", KW_fold);("unfold", KW_unfold);("struct", KW_struct);("variant", KW_variant);("Bool", KW_Bool);("Nat", KW_Nat);("Unit", KW_Unit)]
